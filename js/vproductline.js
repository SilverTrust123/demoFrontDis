...
// --- 2. UI 參數配置分組 ---
const UI_GROUPS = [
    { name: "輸送帶1", items: [{ id: "T14", label: "等待工件時間" }, { id: "T0",  label: "輸送帶停止時間" }] },
    { name: "輸送帶2", items: [{ id: "T7",  label: "感測到工件時間" }] },
    { name: "龍門", items: [
        { id: "T30", label: "機械臂向下等待時間" }, { id: "T3",  label: "機械臂確保向下時間" },
        { id: "T4",  label: "夾取工件等待時間" }, { id: "T31", label: "機械臂到左側停止時間" },
        { id: "T5",  label: "機械臂向下後停止時間" }, { id: "T6",  label: "放開後等待時間" },
        { id: "T15", label: "歸位前等待時間" }
    ]},
    { name: "旋轉缸", items: [
        { id: "T8",  label: "機械臂向下停止時間" }, { id: "T9",  label: "真空等待時間" },
        { id: "T32", label: "旋轉等待時間" }, { id: "T10", label: "機械臂向下等待時間" },
        { id: "T11", label: "放真空等待時間" }
    ]},
    { name: "滑台缸", items: [
        { id: "T40", label: "機械臂向下停止時間" }, { id: "T12", label: "夾抓夾取後等待時間" },
        { id: "T13", label: "放下工件後等待時間" }
    ]}
];
...
// ⚡ 更新左上角「真實產能」
function updateUPH() {
    let cycleTime = calculateCycleTime(false); // 使用 original 計算
...
async function fetchInitialPLCData() {
    try {
        ...
        if (res.ok) {
            ...
            updateUPH(); // 資料抓完後重新精算產能
        }
    } catch (e) {
        console.warn("無法取得初始 PLC 資料，將以預設值顯示。", e);
        updateUPH();
    }
}
...
async function loadModels(fileList) {
    ...
    if (badge) badge.innerText = `正在載入 3D 模型...`;
...
function updateStateTargets(s) {
    ...
    if (PLC_STATES[s]) {
        ...
        timeBox.innerHTML = `移動：<span style="color:#00ff88">${(currentAnimDurationMs/1000).toFixed(2)}s</span> | 等待：<span style="color:#ffaa00">${(currentWaitDurationMs/1000).toFixed(2)}s</span><br>總計執行：${totalTimeSec.toFixed(2)}s`;
    }
...
document.getElementById('btn-sim-metal').addEventListener('click', (e) => {
    if (!parts.p481) return alert("請先等待模型載入完成！");
    const btn = e.target;
    
    if (isSimulating && activeSequence === seqMetalFull) {
        isSimulating = false;
        btn.classList.remove('active');
        btn.innerText = "▶ 模擬金屬產線";
        ...
        btn.innerText = "⏹ 停止金屬模擬";
        
        document.getElementById('btn-sim-nonmetal').classList.remove('active');
        document.getElementById('btn-sim-nonmetal').innerText = "▶ 模擬非金屬產線";
...
document.getElementById('btn-sim-nonmetal').addEventListener('click', (e) => {
    ...
    if (isSimulating && activeSequence === seqNonMetalFull) {
        ...
        simIndex = 0;
        btn.classList.add('active');
        btn.innerText = "⏹ 停止非金屬模擬";
        
        document.getElementById('btn-sim-metal').classList.remove('active');
        document.getElementById('btn-sim-metal').innerText = "▶ 模擬金屬產線";
