...
const STATE_STAGNATE_LIMIT = 30000; // 超過 30 秒步驟沒變動視為停滯過久
...
function updateDarkModeBtnUI() {
    ...
    if (darkModeBtn) {
        ...
        if (isEnglishPage()) {
            ...
            darkModeBtn.innerText = currentTheme === 'dark' ? ' 切換為淺色模式' : ' 切換為深色模式';
        }
    }
}

/* ==============================================================
 * 多國語言：跨資料夾 (html <-> enghtml) 跳轉機制
 * ============================================================== */
...
function updateDNDBtnUI() {
    ...
    if (dndBtn) {
        ...
        if (isEnglishPage()) {
            ...
            dndBtn.innerHTML = `<span style="font-size: 22px; margin-right: 10px;">${isDND ? '🔕' : '✋'}</span> ${isDND ? '勿擾模式 (已開啟)' : '勿擾模式'}`;
        }
...
function initControlButtons() {
    ...
    if (!btnStop || !btnStart) {
        ...
        if (brandLogo && headerRow) {
            ...
            const stopText = isEng ? 'STOP' : '急停';
            const startText = isEng ? 'START' : '繼續';
...
async function handleEStop() {
    if (!confirm(isEnglishPage() ? "Are you sure you want to execute Emergency Stop?" : "確定要執行緊急停止嗎？")) return;
    try {
        const response = await fetchWithAuth(`${CONFIG.API_BASE}/plc/EStop`, { method: 'GET' });
        if (response.ok) {
            const result = await response.json();
            alert(`[${isEnglishPage() ? 'E-STOP Success' : '急停成功'}] ${result.reply || '機台已緊急停止'}`);
        } else {
            const errData = await response.json().catch(() => ({}));
            alert(`[${isEnglishPage() ? 'E-STOP Failed' : '急停失敗'}] ${errData.message || response.statusText}`);
        }
    } catch (error) {
        console.error("急停請求錯誤:", error);
        window.showError("後端連接異常：無法發送急停指令！");
    }
}
...
async function handleResumeStart() {
    const token = localStorage.getItem(CONFIG.AUTH_KEY);
    if (!token) {
        alert(isEnglishPage() ? "Access Denied: Please log in as Administrator first." : "權限不足：請先登入管理員帳號再進行機台控制。");
        document.getElementById('login-modal-overlay').style.display = 'block';
        return;
    }

    if (!confirm(isEnglishPage() ? "Are you sure you want to resume machine operation?" : "確定要解除急停並讓機台繼續運行嗎？")) return;
    ...
    try {
        ...
        if (response.ok) {
            const result = await response.json();
            alert(`[${isEnglishPage() ? 'Resume Success' : '繼續運行成功'}] ${result.message || result.reply || '指令發送成功'}`);
        } else {
            const errData = await response.json().catch(() => ({}));
            alert(`[${isEnglishPage() ? 'Resume Failed' : '繼續失敗'}] ${errData.message || response.statusText}`);
        }
    } catch (error) {
        console.error("繼續請求錯誤:", error);
    }
}
        ...
        try {
            ...
            if (!isAlertActive && (now - lastAlertTime > CONFIG.ALERT_COOLDOWN)) {
                ...
                if (loadRes === "DISCONNECTED" || sensorRes === "DISCONNECTED") {
                    window.showError("後端連接異常：伺服器連線中斷或逾時！");
                    return;
                }
                // 2. 後端只要有一個 block
                if (loadRes && loadRes.block === true) {
                    window.showError("後端防護異常：偵測到後端 Block 狀態觸發！");
                    return;
                }
                // 3. PLC 離線
                if (plcRes === "DISCONNECTED") {
                    window.showError("PLC 離線：無法與控制器通訊！");
                    return;
                }
                // 4. 感測器異常 / 離線
                    ...
                    if (sensorRes.temp && Array.isArray(sensorRes.temp)) {
                        for (const item of sensorRes.temp) {
                            if (item.status === "offline" || item.error) {
                                window.showError(`感測器異常：溫度感測器 ${item.deviceId} 離線或故障！`);
                                return;
                            }
                        }
                    }
                ...
                if (loadRes && loadRes.stability && loadRes.stability < 70) {
                    window.showError(`產線穩定性不穩！目前產線穩定度僅剩 ${loadRes.stability}%`);
                    return;
                }
                // 6. 單一步驟停滯過久
                if (plcRes) {
                    ...
                    if (currentState !== null && currentState !== undefined) {
                        if (currentState === lastPlcState) {
                            if (now - stateChangeTimestamp > STATE_STAGNATE_LIMIT) {
                                window.showError(`設備異常：單一步驟 (State: ${currentState}) 停滯過久未切換！`);
                                return;
                            }
                        } else {
            ...
            if (!isToastActive && (now - lastToastTime > CONFIG.ALERT_COOLDOWN)) {
                ...
                if (camInfo && camInfo.personCount > 0) {
                    window.showToast(`警示：生產線偵測到有人員接近！\n(目前人數：${camInfo.personCount})`);
                    return;
                }
                ...
                try {
                    ...
                    if (lastMetalCount !== null && curMetal >= 1000 && lastMetalCount < 1000) {
                        window.showToast("警示：金屬產線已達到目標加工數量 (1000)！");
                        return;
                    }
                    if (lastNonMetalCount !== null && curNonMetal >= 1000 && lastNonMetalCount < 1000) {
                        window.showToast("警示：非金屬產線已達到目標加工數量 (1000)！");
                        return;
                    }
                ...
                if (sensorRes && sensorRes.temp && borderRes) {
                    ...
                    for (const item of sensorRes.temp) {
                        ...
                        if (item.temperature > currentMax) {
                            window.showToast(`警示：${item.deviceId || '感測器'} 設定溫度異常超標！\n目前：${item.temperature}°C (上限：${currentMax}°C)`);
                            return;
                        }
                    }
                }
                ...
                if (sensorRes && sensorRes.timeError) {
                    window.showToast("警示：操作時間參數調整異常！");
                    return;
                }

                // 5. 通用 Alarm 警示攔截
                if (sensorRes && sensorRes.alarm) {
                    window.showToast(`警示 (Alarm)：收到系統回報的警告訊息！`);
                    return;
                }
            }
...
async function handleLoginSubmit() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            window.showToast("警示：帳號密碼輸入錯誤，請重新嘗試！");
...
function checkLoginStatus() {
    ...
    if (token && loginTime) {
        ...
        if (now - loginTime > CONFIG.EXPIRE_TIME) {
            ...
            window.showToast("警示：登入授權時間已超過，請重新登入！");
            showTimeoutModal(); 
            return;
        }
    }
...
function initSettingsSidebar() {
    ...
    if (!document.getElementById('global-settings-btn')) {
        navMenu.insertAdjacentHTML('beforeend', `
            ...
            <a href="javascript:void(0)" class="nav-status-icon-btn" id="login-status-icon-btn" title="登入狀態" style="display: flex; align-items: center; justify-content: center; padding: 0 10px;">
