document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Add unified DEFAULT_PARAMS to align baseline with other pages (resolves calculation issue of 99)
    ...
    // ⚡ Keep metal sequence only (S1~S24) for capacity calculation
    ...
    if (!confirmBar) {
        ...
        confirmBar.innerHTML = `
            <span class="confirm-text" style="color:white; font-size:16px; font-weight:bold;">Values Changed&nbsp;&nbsp;Expected Capacity ➔ <span id="new-uph" style="color: #00ff88; font-size: 18px;">--</span></span>
    ...
    const controllers = [];
    const paramNames = ["T30", "T3", "T4", "T31", "T5", "T6", "T15"];
    ...
    // ⚡ Calculate and display metal output only
    function updateUPH() {
        ...
        if (uphElement) uphElement.innerText = metalUPH;
    }

    // ⚡ Calculate and display new metal predicted capacity only
    function checkChanges() {
        ...
        if (hasChanged) {
            ...
            if (newUphEl) newUphEl.innerText = newMetalUPH;
    ...
    async function fetchSingleParamData(ctrl) {
        try {
            ...
            console.log(`[Fallback] Failed to read single parameter ${ctrl.param}`);
        }
    }
    ...
    async function fetchLatestData() {
        ...
        try {
            ...
            console.log("ALL API request failed, falling back to single parameter API...");
        }
