document.addEventListener('DOMContentLoaded', () => {
    ...
    // Sensor ID mapping for API routes, real-time data keys, and field configurations
    const SENSOR_CONFIG = {
        '8266_2': {
            endpoint: '/history/temparatureAndHumidityHistory',
            realtimeKey: 'temp',
            type: 'temp_hum',
            headers: ['Timestamp', 'Device ID', 'Temperature (°C)', 'Humidity (%)']
        },
        '8266_4': {
            endpoint: '/history/temparatureAndHumidityHistory',
            realtimeKey: 'temp',
        ...
        '8266_5': {
            ...
            realtimeKey: 'circuit',
            type: 'circuit',
            headers: ['Timestamp', 'Device ID', 'Current (A)']
        },
        '8266_3': {
            endpoint: '/history/airQualityHistory',
            realtimeKey: 'co2',
            type: 'air_quality',
            headers: ['Timestamp', 'Device ID', 'CO2 (ppm)']
        },
        '8266_6': {
            endpoint: '/history/airParticulatesHistory',
            realtimeKey: 'aq',
            type: 'pm25',
            headers: ['Timestamp', 'Device ID', 'Particulate Matter (μg/m³)']
        }
    };
    ...
    // Set date display and trigger query
    ...
    // 3. Dynamically Update Table Header <thead>
    ...
    // 4. Generate Table Row HTML Based on Sensor Type
    ...
    // 5. Render Data Table Content
    function renderTable(dataList, config) {
        const colSpan = config ? config.headers.length : 4;

        if (!Array.isArray(dataList) || dataList.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="${colSpan}" style="text-align: center; color: #888;">No historical data found</td></tr>`;
            return;
        }
    ...
    // 6. Prepend Latest Polled Data to Top of Table
    ...
    // 7. Data Retrieval and Polling Flow
    async function fetchDataAndRender() {
        ...
        tableBody.innerHTML = `<tr><td colspan="${config.headers.length}" style="text-align: center;">Loading data...</td></tr>`;
